...
ACdimmer dimmer(4); //initialase port for dimmer: name(PinNumber);
...
void setup() {
  Serial.begin(9600);
  dimmer.begin(ON); //dimmer initialisation: name.begin(MODE, STATE) 
  dimmer2.begin(ON); //dimmer initialisation: name.begin(MODE, STATE)
}

void loop() 
{
  outVal = map(analogRead(0), 0, 1023, 100, 0); // analogRead(analog_pin), min_analog, max_analog, 100%, 0%);
  outVal2 = map(analogRead(1), 0, 1023, 100, 0); // analogRead(analog_pin), min_analog, max_analog, 100%, 0%);
  Serial.println(outVal); 
  Serial.println(outVal2);
  dimmer.setPower(outVal); // name.setPower(0%-100%)
  dimmer2.setPower(outVal2); // name.setPower(0%-100%)
}
